...
input = tf.keras.Input(shape=(760,), name='input')
allowed_moves = tf.keras.Input(shape=(cardshandedtoeachplayer*amountofbatches,), name='allow')
middle = tf.keras.layers.Dense(10, activation=tf.nn.tanh, name='middle')(input)
cardprobability = tf.keras.layers.Dense(cardshandedtoeachplayer*amountofbatches, activation=tf.nn.sigmoid, name='cardprobability')(middle)
winprobability = tf.keras.layers.Dense(1, activation=tf.nn.tanh, name='batchprobability')(middle)
allowedcardprobability = tf.keras.layers.Multiply(name='final')([allowed_moves, cardprobability])
...
for item in range(0, 100000):
            ...
            for cardnumber in range(0, cardshandedtoeachplayer):
                ...
                for playernumber in range(0, playeramount):
                    ...
                    if playernumber == 0:
                        ...
                        prediction = models[0].predict_on_batch(
                ...
                for playernumber in range(0, playeramount):
                    ...
                    if max(differencetocard) < 0:
                        ...
                        for difference in range(0, len(differencetocard)):
                            if differencetocard[difference] < 0:
                                differencetocard[difference] = cardamount + 1
                    ...
                    selectedcards[playeroflowestcard] = cardamount + 1
    ...
    for cardnumber in range(0, cardshandedtoeachplayer):
        ...
        for playernumber in range(0, playeramount):
            ...
            not_allow_vector = [0] * cardnumber
            allow_vector.extend(not_allow_vector)
            prediction = models[playernumber].predict_on_batch({'input': np.array([situtationvector]), 'allow': np.array([allow_vector]).astype(float)})
            cardpredicion = prediction[0].numpy()
            batchpredicion = prediction[1].numpy()
            indices = [i for i, x in enumerate(playercards[playernumber]) if x == 1]
            for cardpredictionnumber in range(0, len(cardpredicion[0])):
    ...
    for playernumber in range(0, playeramount):
        print(models[playernumber].train_on_batch({'input': np.array(inputexport), 'allow': np.array(allowexport)}, {'final': np.array(cardprobabilityexport), 'batchprobability': np.array(batchprobabilityexport)}))
